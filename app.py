...
def search_church():
    ...
    try:
        ...
        closest_match = (
            churches["Local Name"]
            .apply(lambda x: SequenceMatcher(None, x, query).ratio())
            .idxmax()
        )
        ...
        if artifact_info is None:
            return render_template(
                "index.html", message=f"Nessun risultato trovato per: {query}"
            )
        ...
        return render_template(
            "index.html", message="Errore durante il processo di ricerca."
        )
    ...
    try:
        ...
        if reperto_url or reperto_scritte:  # Verifica se abbiamo trovato dati utili
            return render_template(
                "clickReperto.html",
                reperto=query,
                scritte=reperto_scritte,
                immagini=reperto_url,
                traduzione=reperto_traduz,
                condizione=reperto_condition,
                length=reperto_length,
                width=reperto_width,
                material=reperto_material,
                shape=reperto_shape,
                type=reperto_type,
                query=query,
            )
            ...
            return render_template(
                "index.html", message="Nessun dato disponibile per il codice inserito."
            )
...
def search():
    ...
    closest_match = (
        churches["Local Name"]
        .apply(lambda x: SequenceMatcher(None, x, church_name).ratio())
        .idxmax()
    )
    church_info = churches.iloc[closest_match]
